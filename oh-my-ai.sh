...
deploy_ollama_docker() {
    ...
    if docker ps -a --format '{{.Names}}' | grep -q '^ollama
...
Respond with ONLY the intent word."
    ...
    # Call Ollama with the context
    local response=$(printf '%s' "$intent_prompt" | curl -s -X POST "$OLLAMA_HOST/api/generate" \
        -H "Content-Type: application/json" \
        -d "{\"model\": \"$OLLAMA_MODEL\", \"prompt\": $(printf '%s' "$intent_prompt" | jq -Rs .), \"stream\": false}" \
        2>/dev/null | jq -r '.response' 2>/dev/null | tr -d '\n' | tr '[:lower:]' '[:upper:]' | xargs)
    
    # Fallback to ollama CLI if curl fails
    if [ -z "$response" ] && command -v ollama &> /dev/null; then
        response=$(printf '%s' "$intent_prompt" | ollama run "$OLLAMA_MODEL" 2>/dev/null | tr -d '\n' | tr '[:lower:]' '[:upper:]' | xargs)
    fi
...
"
                ...
                # Call Ollama API
                local api_response=$(printf '%s' "$prompt" | curl -s -X POST "$OLLAMA_HOST/api/generate" \
                    -H "Content-Type: application/json" \
                    -d "{\"model\": \"$OLLAMA_MODEL\", \"prompt\": $(printf '%s' "$prompt" | jq -Rs .), \"stream\": false}" \
                    2>/dev/null | jq -r '.response' 2>/dev/null)
                
                # Fallback to CLI
                if [ -z "$api_response" ] && command -v ollama &> /dev/null; then
                    api_response=$(printf '%s' "$prompt" | ollama run "$OLLAMA_MODEL")
                fi
...
Respond with ONLY the command, no explanation."
            ...
            local cmd=$(printf '%s' "$build_prompt" | curl -s -X POST "$OLLAMA_HOST/api/generate" \
                -H "Content-Type: application/json" \
                -d "{\"model\": \"$OLLAMA_MODEL\", \"prompt\": $(printf '%s' "$build_prompt" | jq -Rs .), \"stream\": false}" \
                2>/dev/null | jq -r '.response' 2>/dev/null)
            
            if [ -z "$cmd" ] && command -v ollama &> /dev/null; then
                cmd=$(printf '%s' "$build_prompt" | ollama run "$OLLAMA_MODEL")
            fi
...
Be concise but thorough."
            ...
            printf '%s' "$prompt" | curl -s -X POST "$OLLAMA_HOST/api/generate" \
                -H "Content-Type: application/json" \
                -d "{\"model\": \"$OLLAMA_MODEL\", \"prompt\": $(printf '%s' "$prompt" | jq -Rs .), \"stream\": false}" \
                2>/dev/null | jq -r '.response' 2>/dev/null || \
                (command -v ollama &> /dev/null && printf '%s' "$prompt" | ollama run "$OLLAMA_MODEL")
            ;;
...
Analyze their history, files, and context to provide relevant help. If they're asking about previous commands, refer to the numbered history entries. Be concise and practical."
            ...
            printf '%s' "$full_prompt" | curl -s -X POST "$OLLAMA_HOST/api/generate" \
                -H "Content-Type: application/json" \
                -d "{\"model\": \"$OLLAMA_MODEL\", \"prompt\": $(printf '%s' "$full_prompt" | jq -Rs .), \"stream\": false}" \
                2>/dev/null | jq -r '.response' 2>/dev/null || \
                (command -v ollama &> /dev/null && printf '%s' "$full_prompt" | ollama run "$OLLAMA_MODEL")
            ;;
    esac
...
if ! check_ollama; then
    # First time setup or Ollama not accessible
    if ! setup_ollama; then
        echo -e "${RED}✗ AI assistant cannot start without Ollama${NC}"
        return 1
    fi
    echo ""
fi
...
if check_ollama; then
    ...
    echo -e "${BLUE}Ollama host:${NC} $OLLAMA_HOST"
    echo ""
...
if [ "$SHELL_TYPE" = "bash" ]; then
    ...
    echo -e "${CYAN}Bash tab completion enabled${NC}"
        ...
        if docker ps --format '{{.Names}}' | grep -q '^ollama
